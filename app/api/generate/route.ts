...
function buildPrompt(
  ...
  const specialInstructionsBlock = specialInstructions
    ? `\nSpecial instructions from the candidate (apply these too):\n"""\n${specialInstructions}\n"""\nFollow these as long as they don't conflict with the no-fabrication rule below — cosmetic/presentation requests (e.g. use a nickname, swap a link, change emphasis, tone adjustments) are fine since the candidate is directing changes to their own information; do not use them as license to invent employers, titles, dates, or metrics that aren't in the source resume.\n`
    : "";
...
- "tailoredResume": the full rewritten resume per the ATS formatting rules above, tailored to the job description, based only on the candidate's actual resume content (at least 50 characters). Write this LAST.
${
  provider === "claude"
