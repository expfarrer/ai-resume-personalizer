...
- After the header, use this exact section order, each as a "## " markdown heading, including only sections that have real content from the source resume (skip any section the source resume has nothing for):
  ...
  3. "## Professional Experience" (or "## Experience") — for each role, a "### " sub-heading line combining title, company, location, and date range (e.g. "### Job Title | Company, Location - Mon YYYY - Mon YYYY or Present"), followed by "- " bullets for that role's accomplishments. The company field is required — an ATS's job-title parser fails when it's missing. If the source resume shows freelance/self-employed/independent-contractor work with no formal employer name, still fill that field in as "Self-Employed" or "Freelance" (e.g. "### Consultant | Self-Employed, Berlin, Germany - 2018 - Present") rather than dropping straight to the location.
  ...
  6. "## Education" — one line per degree, formatted the same way as Experience entries so an ATS can cleanly split degree from institution: "Degree, Major | Institution, Location - Year" (e.g. "Bachelor of Arts (B.A.), German Studies | University of Hamburg, Hamburg, Germany - 2012"). Spell out the degree type (Bachelor of Arts, not just a bare "B.A.") when the source resume makes that clear; include the graduation year whenever the source states it.
